# SimpleAwait repository instructions

Before making code changes, read and follow `/AGENTS.md`.

`/AGENTS.md` is the top-level operating contract and points to the normative SimpleAwait documents under `/docs/simpleawait/`.

For substantial work on the library, use:
- `/.github/prompts/PRIMARY_AGENT_PROMPT.md`

For any substantial change, obtain an independent review using:
- `/.github/agents/REVIEW.md`

Do not change the frozen V1 public API without first updating the normative specification documents as required by `AGENTS.md`.
