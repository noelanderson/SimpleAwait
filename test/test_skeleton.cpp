// Skeleton test.
//
// Validates the build-skeleton contract:
...
//
// This test deliberately uses only the standard coroutine primitives to prove
// the toolchain and public header cooperate.
...
static_assert(SIMPLEAWAIT_MAX_TASKS >= 1, "default task capacity must be >= 1");
static_assert(SIMPLEAWAIT_FRAME_POOL_BYTES >= 1, "default frame pool must be positive");
static_assert(SIMPLEAWAIT_ENABLE_DIAGNOSTICS == 0, "diagnostics default off");
static_assert(SIMPLEAWAIT_ENABLE_ISR == 0, "ISR bridge default off");

static_assert(SIMPLEAWAIT_VERSION_MAJOR == 1, "version major");
static_assert(SIMPLEAWAIT_VERSION_MINOR == 0, "version minor");
static_assert(SIMPLEAWAIT_VERSION_PATCH == 0, "version patch");
static_assert(simpleawait::version_major == SIMPLEAWAIT_VERSION_MAJOR, "version constant");
static_assert(simpleawait::version_minor == SIMPLEAWAIT_VERSION_MINOR, "version constant");
static_assert(simpleawait::version_patch == SIMPLEAWAIT_VERSION_PATCH, "version constant");
...
int main() {
    ...
    SmokeCoro c = smoke();
    SA_CHECK(static_cast<bool>(c.handle));
    SA_CHECK(!c.handle.done());
    SA_CHECK(!g_body_ran);
